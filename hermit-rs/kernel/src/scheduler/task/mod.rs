...
impl BlockedTaskQueue {
	...
	pub fn custom_wakeup(&mut self, task: TaskHandle) -> Option<Rc<RefCell<Task>>> {
		...
		// The task was not in the blocked list: another wake source (e.g. a
		// timeout timer and socket readiness both firing before the task is
		// scheduled) already woke it. A redundant wake is a no-op, not a bug.
		None
	}
