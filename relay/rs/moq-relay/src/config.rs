...
use crate::{AuthConfig, ClusterConfig, StatsConfig};
#[cfg(not(target_os = "hermit"))]
use crate::{InternalConfig, WebConfig};
...
impl Config {
	...
	pub fn load() -> anyhow::Result<Self> {
		...
		{
			// uhyve forwards no argv; configure inline.
			#[allow(unused_mut, deprecated)]
			let mut config = Self::parse_and_merge(std::iter::once(std::ffi::OsString::from("moq-relay")))?;
			config.server.bind = Some("0.0.0.0:443".to_string());
			config.client.bind = "0.0.0.0:9443".parse().unwrap();
			config.client.tls.disable_verify = Some(true);
			// Browser-facing TLS: use a real cert if provided (HERMIT_TLS_CERT/KEY), else self-signed.
			if let Ok(cert) = std::env::var("HERMIT_TLS_CERT") {
				config.server.tls.cert = vec![std::path::PathBuf::from(cert)];
				if let Ok(key) = std::env::var("HERMIT_TLS_KEY") {
					config.server.tls.key = vec![std::path::PathBuf::from(key)];
				}
				config.server.tls.generate = vec![];
			} else {
				config.server.tls.generate = vec!["localhost".to_string()];
			}
			if let Ok(u) = std::fs::read_to_string("/certs/upstream") {
			...
			#[cfg(feature = "iroh")]
			{
				...
				// Per-guest identity + federation target, so one binary is both origin and edge.
				config.iroh.secret = Some(std::env::var("HERMIT_IROH_SECRET").unwrap_or_else(|_| {
					"b1b1b1b1b1b1b1b1b1b1b1b1b1b1b1b1b1b1b1b1b1b1b1b1b1b1b1b1b1b1b1b1b1".to_string()
				}));
				config.iroh.bind_v4 = Some("0.0.0.0:9500".parse().unwrap());
				config.iroh.disable_relay = Some(true);
				// HERMIT_CLUSTER_CONNECT: empty string -> origin (no dial); unset -> legacy default.
				config.cluster.connect = match std::env::var("HERMIT_CLUSTER_CONNECT") {
					Ok(cc) if cc.is_empty() => vec![],
					Ok(cc) => vec![cc],
					Err(_) => {
						vec!["iroh://2d8c2200111923090e7ca30487b5e1ac533f6fe31be40347330ee1f6b9828050/".to_string()]
					}
				};
			}
			config.log.init()?;
