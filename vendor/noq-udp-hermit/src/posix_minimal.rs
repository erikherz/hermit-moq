...
impl UdpSocketState {
    ...
    pub fn recv(
        ...
        {
            // hermit: no recvmsg/vectored datapath; single-buffer recv_from (BATCH_SIZE == 1)
            let first = &mut bufs[0];
            let raw: &mut [u8] = &mut *first;
            let uninit: &mut [std::mem::MaybeUninit<u8>] =
                unsafe { &mut *(raw as *mut [u8] as *mut [std::mem::MaybeUninit<u8>]) };
            let (len, addr) = socket.0.recv_from(uninit)?;
            // hermit has no IP_PKTINFO/recvmsg cmsg datapath, so recover the local
            // destination IP from the socket's bound address when it is specified.
            // iroh/quinn need a concrete dst_ip to admit an incoming connection on a
            // v4-only stack (no v6 dualstack available on hermit).
            let dst_ip = socket
                .0
                .local_addr()
                .ok()
                ...
                // Report the *canonical* local IP (v4 stays v4). iroh's IP transport
                // send-path validity check requires the transmit src_ip family to match
                // the sending socket's family; a v6-mapped (::ffff:) local is classed as
                // IpAddr::V6 and the v4 sender rejects it -> every server->client datagram
                // gets blackholed. Canonical v4 keeps the family consistent on send.
