...
impl Future for EndpointDriver {
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        let (has_incoming, stop, keep_going) = {
            ...
            // Note: we intentionally do NOT self-reschedule via
            // `cx.waker().wake_by_ref()` when there is more work. On Hermit's
            // current-thread runtime a self-wake issued during a task's own poll
            // does not reliably re-poll the task AND prevents the runtime from
            // parking, which wedges the whole runtime (timers never fire) once the
            // peer goes idle. A single pass per poll is sufficient: fresh inbound
            // datagrams re-poll us via socket readiness, and pending timers are
            // driven by their own tasks once the runtime is allowed to park.
            // Drain the receive path to WouldBlock (see below): tokio only re-arms
            // the socket readiness waker when a recv returns WouldBlock. Reading one
            // datagram and stopping while the socket is still readable leaves tokio
            // with "already-ready" readiness and NO registered waker, so this task is
            // never re-polled and buffered datagrams stall unread (observed: smoltcp
            // readable 305k times, app read only 3). Loop drive_recv until it reports
            // no more (keep_going == false = the WouldBlock read that arms the waker).
            // Stop early on a fresh Incoming so the spawned accept() can pop it first.
            // Bounded drain that YIELDS to the scheduler. A full drain-to-WouldBlock
            // arms tokio's readiness waker and, on Hermit's current_thread runtime, the
            // resulting IO-woken re-polls MONOPOLIZE the scheduler -> the Notify-woken
            // accept() task and timer tasks starve (observed build72: CONNDRV=0, HB dies
            // at 11, accept() polled once). Single-pass keeps accept()+timers healthy
            // (build63: POP=1, HB=40) but under-delivers buffered datagrams. Compromise:
            // read a SMALL batch per poll to make progress on delivery, then STOP before
            // WouldBlock so we do NOT arm the storming readiness waker -- the next inbound
            // datagram's readiness edge (level-triggered poll(2)) re-polls us to drain the
            // rest, one batch at a time, without starving the other tasks.
            // Quinn-idiomatic: one bounded recv cycle per poll. drive_recv's
            // internal loop drains until either WouldBlock (keep_going == false,
            // which arms tokio's readiness waker) or the per-cycle work limiter
            // trips (keep_going == true, and NO waker is armed -> we must
            // re-schedule ourselves to finish draining). Self-waking ONLY when
            // keep_going is true is bounded: once the socket is truly drained we
            // return Pending with the waker armed and the runtime is free to park
            // (timers fire, heartbeat stays alive). This interleaves fairly --
            // the self-wake enqueues us at the back of the run queue, so the
            // Notify-woken accept() task and timer tasks are polled first.
            let keep_going = endpoint.drive_recv(cx, now)?;
            endpoint.handle_events(cx, &self.0.shared);

            let has_incoming = !endpoint.recv_state.incoming.is_empty();
            let stop = endpoint.recv_state.connections.is_empty()
                && (self.0.shared.ref_count.load(Ordering::Relaxed) == 0
                    || endpoint.recv_state.connections.close.is_some());
            (has_incoming, stop, keep_going)
        };
        ...
        if keep_going {
            // More datagrams remain than the per-cycle work budget allowed. The
            // recv path did NOT arm a readiness waker on this exit path, so
            // re-schedule ourselves to continue draining on the next scheduler
            // tick. Bounded: keep_going is false once the socket is drained.
