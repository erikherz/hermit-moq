...
impl From<&AuthError> for http::StatusCode {
	fn from(err: &AuthError) -> Self {
		match err {
			...
			AuthError::ApiInvalidResponse(_) => http::StatusCode::BAD_GATEWAY,
...
impl KeyResolver {
	...
	#[cfg(target_os = "hermit")]
	async fn fetch_key(_client: &ClientWithMiddleware, _url: url::Url) -> Result<Arc<Key>, AuthError> {
...
impl Auth {
	...
	#[cfg(target_os = "hermit")]
	async fn fetch_public_response(_client: &ClientWithMiddleware, _url: &url::Url) -> Result<PublicResponse, AuthError> {
		Err(AuthError::KeyNotFound)
	}
	...
	pub async fn verify(&self, params: &AuthParams) -> Result<AuthToken, AuthError> {
		...
		if let (Some(token), Some(keys)) = (params.jwt.as_deref(), asym_keys()) {
			...
			let claims = moq_token::Claims {
				...
				expires: c.expires.map(|e| std::time::UNIX_EPOCH + std::time::Duration::from_secs(e.max(0) as u64)),
				..Default::default()
			};
	...
	#[cfg(target_os = "hermit")]
	fn build_client(_tls: &rustls::ClientConfig) -> anyhow::Result<ClientWithMiddleware> {
		Ok(())
	}
}
