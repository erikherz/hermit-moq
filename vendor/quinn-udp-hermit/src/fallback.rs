...
impl UdpSocketState {
    ...
    pub fn recv(
        ...
        // hermit/socket2-0.5: no recv_from_vectored; recv into first buffer (BATCH_SIZE==1)
        let buf0: &mut [u8] = &mut bufs[0];
        let uninit = unsafe { &mut *(buf0 as *mut [u8] as *mut [core::mem::MaybeUninit<u8>]) };
