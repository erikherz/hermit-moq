...
pub(crate) fn claim_initial_heap() {
	#[repr(C, align(0x1000))]
	// Enlarged 4 KiB -> 256 KiB: env::init() parses all uhyve env vars into a
	// HashMap<String,String> on THIS heap before the main heap is claimed; longer
	// env sets (e.g. an iroh:// cluster-connect URL) overflowed the old 4 KiB and
	// OOM-panicked ~50% of boots before "Jumping into application".
	struct InitialHeap([MaybeUninit<u8>; 0x40000]);

	debug_assert_eq!(
		Layout::new::<InitialHeap>(),
		Layout::from_size_align(0x40000, 0x1000).unwrap()
	);
