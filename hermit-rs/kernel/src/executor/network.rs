...
impl<'a> NetworkInterface<'a> {
	...
	pub(crate) fn create_udp_handle(&mut self) -> Result<Handle, ()> {
		// Enlarged UDP rings: the default 4 packet-metadata slots hold at most 4
		// datagrams, so any burst > 4 (e.g. a QUIC handshake flight or a stream
		// window) overflows and smoltcp drops the excess at the device level before
		// the app can drain it. 256 slots + 256 KiB payload absorbs bursts.
