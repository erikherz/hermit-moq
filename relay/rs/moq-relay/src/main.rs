...
async fn main() -> anyhow::Result<()> {
	...
	let (server, client) = {
		...
		// With DHT discovery enabled the endpoint resolves peer addresses itself, so skip
		// the static direct-address injection used by the earlier DNS-less demo.
		#[cfg(target_os = "hermit")]
		let client = if std::env::var("MOQ_IROH_DHT_BOOTSTRAP").map(|s| !s.is_empty()).unwrap_or(false) {
			client.with_iroh(iroh.clone())
		} else {
			client
				.with_iroh(iroh.clone())
				.with_iroh_addrs(vec!["10.9.9.1:48731".parse().unwrap()])
		};
		#[cfg(not(target_os = "hermit"))]
		let client = client.with_iroh(iroh.clone());
		(server.with_iroh(iroh), client)
	};
