...
impl Future for ConnectionDriver {
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        // Drive the connection in an internal loop until there is no more immediate
        // work. Upstream quinn loops here; the noq fork replaced the loop with a
        // `cx.waker().wake_by_ref()` self-reschedule to reduce lock contention on
        // multithreaded runtimes. On Hermit's current-thread runtime that self-wake
        // does NOT reliably re-poll the task (and wedges the runtime), so a fired
        // timer's follow-up transmit never happens. Loop in-place instead.
        ...
        loop {
            ...
            // If a timer expires, there might be more to transmit. When we transmit
            // something, we might need to reset a timer. Hence, we loop until neither
            // happens.
            ...
            __iters += 1;
            if __iters >= 1000 {
                // Safety valve: yield to the runtime rather than spin forever.
...
impl State {
    ...
    fn drive_timer(&mut self, cx: &mut Context<'_>) -> bool {
        ...
        match self.inner.poll_timeout() {
            Some(deadline) => {
                let __now = self.runtime.now();
                // Hermit workaround: registering an already-elapsed (past/zero) deadline
                // with tokio's time driver fails to wake the current-thread runtime and
                // wedges the whole timer wheel (all subsequent timers, incl. unrelated
                // ones like heartbeats, stop firing). Fire immediately instead of arming
                // a doomed tokio timer, and signal keep_going so the driver re-polls.
                if deadline <= __now {
                    self.inner.handle_timeout(__now);
                    self.timer_deadline = None;
                    return true;
                }
                // EXPERIMENT: always create a fresh timer instead of reset(), to test
                // whether Sleep::reset() fails to re-register the timer on Hermit.
