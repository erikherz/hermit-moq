...
impl EndpointConfig {
	pub async fn bind(self) -> Result<Option<Endpoint>> {
		...
		// No-DNS discovery: when MOQ_IROH_DHT_BOOTSTRAP is set (e.g. inside a HermitOS
		// unikernel with no DNS resolver), discover peers over the mainline DHT instead of
		// iroh's default DNS/pkarr services. Bootstrap nodes are passed as ip:port (the host
		// resolves the hostnames since the guest can't). We publish our own address unfiltered
		// (so direct IPs are in the record) and resolve peers the same way.
		let dht_bootstrap = std::env::var("MOQ_IROH_DHT_BOOTSTRAP").ok().filter(|s| !s.is_empty());
		let mut builder = if let Some(bootstrap) = dht_bootstrap.as_deref() {
			let mut dht_builder = n0_mainline::Dht::builder();
			let refs: Vec<&str> = bootstrap.split(',').filter(|s| !s.is_empty()).collect();
			if !refs.is_empty() {
				dht_builder.bootstrap(&refs);
			}
			...
			tracing::info!("iroh discovery: mainline DHT (no DNS)");
			Endpoint::builder(iroh::endpoint::presets::Minimal).address_lookup(dht_lookup)
		} else if self.disable_relay.unwrap_or(false) {
		...
		// NAT-advertise: when the guest is behind a 1:1 DNAT (host public IP:port ->
		// guest), it cannot discover its own reachable address (Minimal preset, no
		// relay/STUN). MOQ_IROH_EXTERNAL_ADDR lets the operator declare it, so the DHT
		// publisher advertises a reachable address that peers can dial through the DNAT.
		if let Ok(ext) = std::env::var("MOQ_IROH_EXTERNAL_ADDR") {
			for a in ext.split(",").filter(|s| !s.is_empty()) {
				match a.parse::<std::net::SocketAddr>() {
					Ok(addr) => {
						builder = builder.external_addr(addr);
						tracing::info!(%addr, "iroh external addr advertised");
					}
					Err(e) => tracing::warn!("bad MOQ_IROH_EXTERNAL_ADDR {a:?}: {e}"),
				}
			}
		}
