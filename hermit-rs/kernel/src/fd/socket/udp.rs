...
/// Allocate an ephemeral UDP port for binds that request port 0 (e.g. iroh binds `[::]:0`).
...
impl ObjectInterface for Socket {
	...
	async fn bind(&mut self, endpoint: ListenEndpoint) -> io::Result<()> {
		...
		// A port of 0 means "assign an ephemeral port" (POSIX). smoltcp cannot bind
		// port 0, so allocate one here. iroh relies on this for its `[::]:0` v6 bind.
		if endpoint.port == 0 {
