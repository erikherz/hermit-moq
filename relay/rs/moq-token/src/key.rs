	...
	fn to_encoding_key(&self) -> crate::Result<&EncodingKey> {
		...
		let encoding_key = match self.key {
			...
			KeyType::OKP {
				...
				match curve {
					EllipticCurve::Ed25519 => {
						// PKCS#8 v1 (RFC 8410) for Ed25519: fixed 16-byte header + 32-byte seed.
						// Avoids a native crypto backend (aws-lc-rs) so this cross-compiles to hermit.
						let mut pkcs8 = vec![
							0x30, 0x2e, 0x02, 0x01, 0x00, 0x30, 0x05, 0x06, 0x03, 0x2b,
							0x65, 0x70, 0x04, 0x22, 0x04, 0x20,
						];
						pkcs8.extend_from_slice(d.as_slice());
